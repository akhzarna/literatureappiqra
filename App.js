...
function HomeScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    axios.get(apiUrl)
      .then(response => {
        const bookData = response.data.data.map(book => ({
          ...book,
          coverPhotoUrl: `http://139.59.177.72/${book.coverPhotoUri}`,
        }));
        ...
        renderItem={({ item }) => (
          ...
          >
            <View style={styles.item}>
              <Image
                source={{ uri: item.coverPhotoUrl }} // Display the cover photo
                style={styles.coverPhoto} // Define styles for the cover photo
              />
